...
C>        ====        ==========        =====================        |
C>     05/11/2016    Joao Ferreira      full network model included  |
C>     05/11/2016    Joao Ferreira      nonaffine network            |
C>     06/11/2016    Joao Ferreira      affine network               |
C>     07/04/2017    Joao Ferreira      contractile affine network   |
C>     16/02/2018    Joao Ferreira      compliant cross-linkers      |
C>--------------------------------------------------------------------
C>     Description:
C>     UMAT: USER MATERIAL FOR THE FULL NETWORK MODEL.
C>                AFFINE AND NON-AFFINE DEFORMATIONS
...
C     CONTRACTILE FILAMENT
     ...
     1                 PREFDIR(NELEM,4),VARACT,SACTISO(NDI,NDI),
     1                 SACTVL(NDI),SACTVC(NDI,NDI),DIRMAX(NDI)
...
C
      ...
      FRIC     = PROPS(18)
      FFMAX    = PROPS(19)
      AFFPROPS = PROPS(15:19)
C     VISCOUS EFFECTS
      VV       = INT(PROPS(20))
      VSCPROPS = PROPS(21:26)
...
C
      ...
      ENDIF
C        READ STATEV
      CALL SDVREAD(FRAC0,RU0,STATEV,VV)       
C----------------------------------------------------------------------
C---------------------------- KINEMATICS ------------------------------
C----------------------------------------------------------------------
...
C        MATERIAL TENSORS WITH ERRORS. ONLY SPATIAL WORKING...
C      CALL AFFMATNETFIC(PKNETFICAF,CMNETFICAF,DISTGR,MF0,RW,FILPROPS,
C     1                         AFFPROPS,RU0,DTIME,FRAC,EFI,DET,NDI)
...
C------------ PASSIVE NETWORK -------------
      IF (NNA.GT.ZERO) THEN
      CALL AFFCLNETFIC(SNETFICAFP,CNETFICAFP,DISTGR,MF0,RW,FILPROPS,
...
C      PKNETFIC=PKNETFICAF+PKNETFICNAF
      SNETFIC=SNETFICAF+SNETFICAFP+SNETFICNAF
C      CMNETFIC=CMNETFICAF+CMNETFICNAF
      CNETFIC=CNETFICAF+CNETFICAFP+CNETFICNAF
C----------------------------------------------------------------------
C     STRAIN-ENERGY
C      SSE=SSEV+SSEISO
...
C      STATEV(1:27) = VISCOUS TENSORS
       CALL SDVWRITE(FRAC,RU0,DET,STATEV,VV,VARACT,DIRMAX)
C     END DO
C----------------------------------------------------------------------
      RETURN
      END
