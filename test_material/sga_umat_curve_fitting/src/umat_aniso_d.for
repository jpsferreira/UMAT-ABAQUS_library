...
C     14/03/2016    Joao Ferreira      k dispersion added           |
C     22/04/2016    Joao Ferreira         local damage              |
C--------------------------------------------------------------------
C--------------------------------------------------------------------
C     Description:
C     UEXTERNALDB: mount database; fibers directions;.inc, .inp files
C     UMAT: mooneyrivlin matrix + Holzapfel fibers + k dispersion
C--------------------------------------------------------------------
...
C                COUNTERS AND STATE INDICATORS
     ...
     1         FLAG1,FLAG2,DSTAT,DSTAT1,DSTAT2
...
C     ISOTROPIC PART
      SEM=C10*(CBARI1-THREE)+C01*(CBARI2-THREE)
C     ANISOTROPIC PART
...
C       HUMPRHEY
C       C     ISOTROPIC PART
C      SEM=C10*(DEXP(C01*(CBARI1-THREE))-ONE)
C     ANISOTROPIC PART
C      SEF1=C3*(DEXP(C4*E1*E1)-ONE)
C      SEF2=C5*(DEXP(C6*EE1*EE1)-ONE)
C
C----------------------------------------------------------------------
...
C----------------------------------------------------------------------
CC    HUMPRHEY
CC     FIRST AND SECOND DERIVATIVES UI - ISOTROPIC CONTRIBUTION
CC
CC  FIRST DERIVATIVE UI WITH RESPECT TO I1
C      DUDI1=C10*C01*DEXP(C01*(CBARI1-THREE))
CC  SECOND DERIVATIVE UI  WITH RESPECT TO I1
C      D2UD2I1=C01*C01*C10*DEXP(C01*(CBARI1-THREE))
CC  FIRST DERIVATIVE UI WITH RESPECT TO I2
C      DUDI2=ZERO
CC  SECOND DERIVATIVE UI  WITH RESPECT TO I2
C      D2UD2I2=ZERO
CC  SECOND DERIVATIVE UI  WITH RESPECT TO I1 AND I2
C      D2DUDI1DI2=ZERO
C----------------------------------------------------------------------
...
CC	FAMILY 1
C
C      IF(E1.GT.ZERO) THEN

C      E3=DEXP(C4*E1*E1)
C      E2=TWO*C3*C4
C      E4=ONE+TWO*C4*E1*E1*BARLAMBDA
C      E5=FOUR*(CBARI4**(THREE/TWO))

C      DUDI4=E2*E1*E3/(TWO*BARLAMBDA)
C      D2UD2I4=E2*E3*E4/E5
...
C      IF(EE1.GT.ZERO) THEN

C      EE3=DEXP(C6*EE1*EE1)
C      EE2=TWO*C5*C6
C      EE4=ONE+TWO*C6*EE1*EE1*BARLAMBDA2
C      EE5=FOUR*(CBARI6**(THREE/TWO))

C      DUDI6=EE2*EE1*EE3/(TWO*BARLAMBDA2)
C      D2UD2I6=EE2*EE3*EE4/EE5
C      ELSE
...
C        -----SPATIAL TANGENT MODULI------
                   DDSIGDDE(I,J,K,L)=(ONE-DMGF1)*DDSDDEFIB1(I,J,K,L)+
     +                               (ONE-DMGF2)*DDSDDEFIB2(I,J,K,L)+
     +                               (ONE-DMGM)*DDSDDEISO(I,J,K,L)
C
